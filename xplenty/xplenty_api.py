    ...
    if date_keys:
        for in_key in date_keys:
            ...
            try:
                ...
                logger.error(f'Failed to parse date [{in_date}]')
                # raise e
                out_date = None
...
class XplentyClient(object):
    ...
    def get(self, url):
        ...
        self._add_authorization_header(request, self.api_key)
    ...
    def post(self, url, data_dict={}):
        ...
        request = Request(url, data=json_data.encode('utf-8'), headers=HEADERS)
        self._add_authorization_header(request, self.api_key)
    ...
    def _add_authorization_header(self, request, api_key: str):
        base64string = base64.b64encode(bytes('%s:' % api_key, 'ascii'))
    ...
    def create_cluster(self, cluster_type, nodes, cluster_name, cluster_description, terminate_on_idle=False,
                       time_to_idle=3600):
        cluster_info = {}
